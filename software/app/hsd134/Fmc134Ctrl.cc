...
#define unitapi_write_register(unit,addr,val) reinterpret_cast<Pds::Mmhw::Reg*>(this)[addr] = val
#define unitapi_read_register(unit,addr,pval) *(pval) = reinterpret_cast<Pds::Mmhw::Reg*>(this)[addr]
...
int32_t Fmc134Ctrl::default_init(Fmc134Cpld& cpld, unsigned mode)
{
        ...
        //
        // Check if this is necessary.  It resets the JESD links which counts errors.
        // This is fine if it always succeeds, which it doesn't.
        //
        {  unsigned av = adc_val;
           unsigned m0 = cpld.readRegister(Fmc134Cpld::ADC0, 0x0205);
           unsigned m1 = cpld.readRegister(Fmc134Cpld::ADC1, 0x0205);
           printf("Fmc134Ctrl::init adcvalid 0x%x  mode %u  mode %u\n", av,m0,m1);
           if (av == 0xf && m0==mode && m1==mode)
               return 0;
        }
