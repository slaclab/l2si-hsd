#include "hsd134/Fmc134Cpld.hh"

#include <unistd.h>
#include <stdio.h>
#include <iostream>

#include "hsd134/SysLog.hh"
...
#define WRREG(r,v) {                                            \
    ...
    if (v&2)
        logging::info("ADC SYSREF Calibration Done\n");
    else
        logging::info("ADC SYSREF Calibration Not Done\n");
...
#define i2c_write(unit,addr,val)  reinterpret_cast<Pds::Mmhw::RegProxy*>(this)[addr]=val
#define i2c_read(unit,addr,valp)  *(valp) = reinterpret_cast<Pds::Mmhw::RegProxy*>(this)[addr]
...
{
    logging::info("*** default_clocktree_init ***\n");
    uint32_t dword;
    uint32_t samplingrate_setting;
    unsigned i2c_unit=0;
    int32_t rc = UNITAPI_OK;     

    logging::info("Configured the sampling rate to 3200MSPs\n");
    ...
    logging::info("*** default_clocktree_init done ***\n");
    return 0;
}
...
int32_t Fmc134Cpld::internal_ref_and_lmx_enable(uint32_t i2c_unit, uint32_t clockmode)
{
    ...
    case CLOCKTREE_CLKSRC_INTERNAL:                         // Internal Reference
        logging::info("Internal Reference Mode \n");
        // Set bits 3, 1, and 0
        dword |= 0xB;
    ...
    case CLOCKTREE_CLKSRC_EXTERNAL:
        logging::info("External Sample Clock \n");
    ...
    case CLOCKTREE_REFSRC_EXTERNAL:         
        logging::info("External Reference Mode \n");
        // turn off 0sc, point at ext ref enable LMX bits 3, 1, and 0
        dword = 0xCC;
        i2c_write(i2c_unit, cpld_address + 2, dword);
        if(rc!=UNITAPI_OK)
    ...
    case CLOCKTREE_REFSRC_STACKED:         
        logging::info("Stacked Internal Reference Sourcing Mode \n");
                                     ...
                                     std::string& calib_adc1)
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // ADC Initializaton
    logging::info("Clearing ADC RESET pins");
    ...
    if ((dword0 & 0x2) == 0x2)
        logging::info("ADC0 SYSREF Calibration Done\n");
    else {
        logging::error("ADC0 SYSREF Calibration NOT Done!\n");
    ...
    if ((dword0 & 0x2) == 0x2)
        logging::info("ADC1 SYSREF Calibration Done\n");
    else {
        logging::error("ADC1 SYSREF Calibration NOT Done!\n");
        return FMC134_ERR_ADC_INIT;
    }
    ...
    case FG_CAL:
        // Set CAL_FG to enable foreground calibration
        spi_write(i2c_unit, ADC_SELECT_BOTH, 0x0062, 0x01);
        break;
    case BG_CAL:
        // Set CAL_BG to enable background calibration
        spi_write(i2c_unit, ADC_SELECT_BOTH, 0x0062, 0x02);
        break;
    default:
        break;
    }
    ...
    logging::info("Read FS_RANGE_0: %x %x %x %x\n",
                  dw[0], dw[1], dw[2], dw[3]);

    for(unsigned i=0; i<4; i++)
        spi_read(i2c_unit, ADC1_SELECT, 0x0030+i, &dw[i]) ;
    logging::info("Read FS_RANGE_1: %x %x %x %x\n",
                  dw[0], dw[1], dw[2], dw[3]);
    ...
    if (cmode==FG_CAL) {
        ...
        spi_write(i2c_unit, ADC_SELECT_BOTH, 0x006C, 0x00); // trigger
        spi_read(i2c_unit, ADC0_SELECT, 0x006C, &dword0);
        logging::info("CAL_TRIG 0x%x\n",dword0);
        spi_write(i2c_unit, ADC_SELECT_BOTH, 0x006C, 0x01); // trigger
        spi_read(i2c_unit, ADC0_SELECT, 0x006C, &dword0);
        logging::info("CAL_TRIG 0x%x\n",dword0);
        for(unsigned ch=0; ch<2; ch++) {
            DevSel adcsel = ch==0 ? ADC0_SELECT : ADC1_SELECT;
            for(unsigned i=0; i<100; i++) {
                unitapi_sleep_ms(500);

                // Read CAL status
                spi_read(i2c_unit, adcsel, 0x006A, &dword0);
                if(rc!=UNITAPI_OK) return rc;
                if ((dword0 & 0x1) == 0x1) {
                    logging::info("ADC%d FG Calibration Done\n",ch);
                    break;
                }
                else {
                    logging::warning("ADC%d FG Calibration NOT Done! [%x]\n",ch,dword0);
                }
            }
            if ((dword0 & 0x1) == 0)
                return FMC134_ERR_ADC_INIT;
    ...
    logging::info("*** default_adc_init done ***\n");
...
std::string Fmc134Cpld::adc_cal_dump(unsigned)
{
  return std::string();
}

void        Fmc134Cpld::adc_cal_load(unsigned,const std::string&)
{
}
